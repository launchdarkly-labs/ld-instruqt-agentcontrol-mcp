...
- [ ] **The LaunchDarkly UI tab does not work, and the reason is architectural.** Parked as of 2026-08-17 — early stage, not blocking the track. Full trace, so nobody has to rediscover it:
  - `instruqt-login-redirector` (Lambda URL, AWS `955116512041`, us-east-2) is the tab target. It polls DynamoDB `instruqt-workshop-production-sandbox` for the `sandboxId`, waits **60s**, then 301s to that row's `LoginUrl`. No row for our sandboxes, so it times out and the Lambda URL returns 502.
  - Rows map `SandboxId` → `{ProjectId, LoginUrl}` where `LoginUrl` is `…/production/login?id=<projectId>&expires=<ms>&token=<hmac>`, and the token is `HMAC-SHA256(key = the user's loginSecret, data = "<projectId>.<expiresAt>")`.
  - That URL is served by `instruqt-workshop-production-api` (HTTP API `nfiaf3w10f`) → `…-user-login-handler`, which is a **SAML IdP**: it verifies the signed URL, then mints a signed SAML Response with `nameIdentifier` = the user's `loginEmail` and posts it to LaunchDarkly's ACS. Issuer, audience, recipient, cert and private key are in SSM under `/instruqt-workshop/production/`.
  - `…-user-created-handler` is a **`"Hello World"` stub** — user provisioning is not implemented there. Something outside both LaunchDarkly GitHub orgs writes the rows; searches for the table name, `SandboxId`, and `login-redirector` found nothing. The login handler *was* invoked several times on 2026-08-17 with no errors, so the pipeline is live for some other workshop.
  - **CORRECTION (2026-08-17): there is no account mismatch.** A diagnostic in `track_scripts/setup-workstation` prints the account the lab provisions into, and it is `652d7d3060d93a12fccd6e2e` — the same account `saml-recipient` points at. An earlier finding claimed otherwise; that came from running `caller-identity` with an unrelated token named `test` on `6502137e3310e112c47aeb90`, not with the `LAUNCHDARKLY_ACCESS_TOKEN` secret. The secret is on the workshops account, exactly as its description says.
  - **So the tab is fixable, and cheaply.** No account migration, no new IdP, no SSO reconfiguration, no trial account. Everything needed already exists in AWS `955116512041`, where you have admin. What is missing for our sandboxes is only:
    1. a user record in the users table (`projectId`, `loginEmail`, `loginSecret`, `loginExpiresAt`), with `loginEmail` a member of `652d7d3060…`;
    2. a row in `instruqt-workshop-production-sandbox` mapping `SandboxId` → `{ProjectId, LoginUrl}`, where `LoginUrl` is `…/production/login?id=<projectId>&expires=<ms>&token=HMAC-SHA256(loginSecret, "<projectId>.<expiresAt>")`.
  - Both could be written from `track_scripts/setup-workstation` using the AWS secrets the track already has. The open question is whether `ProjectId` must be one of the pet-name projects the existing pipeline creates, or whether any project key in the account works — the login handler compares the URL's `id` against the user record's `projectId`, so it looks like any key works as long as the two agree.
  - The `virtualbrowsers` tab is deliberately still in `config.yml`. Nothing in the track depends on it: the score payoff reads back through the agent.
