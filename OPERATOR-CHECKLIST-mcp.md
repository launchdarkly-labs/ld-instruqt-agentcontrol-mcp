...
- [ ] **The LaunchDarkly UI tab does not work, and the reason is architectural.** Parked as of 2026-08-17 — early stage, not blocking the track. Full trace, so nobody has to rediscover it:
  ...
  - **The blocker is account mismatch, not the lambda.** `saml-recipient` is `https://app.launchdarkly.com/trust/saml2/acs/652d7d3060d93a12fccd6e2e` (the labs account). Our `LAUNCHDARKLY_ACCESS_TOKEN` is on `6502137e3310e112c47aeb90`. So even a healthy tab would sign the learner into an account that does not contain their `agentcontrol-intro-<sandbox>` project.
  - **Cheapest fix by far:** get a service token on `652d7d3060…` from whoever owns labs, and provision there. The IdP, table, and redirector already exist and work; only the account is wrong.
  - **Fallback:** stand up our own IdP (keypair, SSM params, table, redirector + login lambda) against an account we admin, and configure SAML SSO there. Half a day to a day, and SAML signing is the part that fails opaquely. Note a trial account expires in ~14 days, so it proves the mechanism but is not a home for it. Note also that LD SSO is account-wide — do not reconfigure it on `6502137e…`, which real people log into.
