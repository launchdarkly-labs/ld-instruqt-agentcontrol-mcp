...
- [ ] **Create the `BEDROCK_ROLE_ARN` Instruqt secret** = `arn:aws:iam::<account>:role/RoleForAccessFromInstruqt` (from `terraform output role_arn` in `gcp-federation/`, or read it off the live role). `track_scripts/setup-workstation` writes `/etc/bedrock-federation.env` from it on every start, so there is nothing to hand-edit on the image. Without the secret the tools still install and MCP still works, but Claude Code has no model and Otto can't answer — you'll see `Could not load credentials from any providers`.
  - `BEDROCK_JWT_AUDIENCE` defaults to `instruqt-agentcontrol` and only needs a secret if the trust policy's `accounts.google.com:oaud` ever changes.
  - `instruqt track push` **fails** while the secret is declared in `config.yml` but missing from the team, with `Entity not found: configured secret ... does not exist`.
